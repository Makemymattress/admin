...
import { PROXY, S3PROXY, Variants_const } from "@/config";
...
const Products = ({ id, setState, setId, state }: any) => {
  ...
  console.log("🚀 ~ Products ~ form:", form);
  ...
  useEffect(() => {
    const getOne = async () => {
      if (id) {
        ...
        const d = findata.data?.data[0];
        setForm(d);
        // setValue(d.description);
        if (d.images?.length) {
          const data = d.images.map((url: any, uid: any) => {
        ...
        if (d.thumbnail) {
          const data = [setDefaultImages(d.thumbnail, 20)];
        ...
        if (d.videos?.length) {
          const data = d.videos.map((url: any, uid: any) => {
            return setDefaultImages(url, uid);
          });
          setVideos(data);
          setVideoLinks(setDefaultImages1(data));
        }
        setThicknessInputs(d["thickness_options"]);
        setThinknessVal(d["thickness_values"]);
        setSelectedThickness(d.thickness);
      }
    };
    getOne();
  }, [r]);
      ...
      <>
        ...
        {state == "edit" ? (
          <>
            ...
            <div className=" p-6.5 gap-2  flex flex-col rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
              {form?.reviews?.map((review: any) => {
                return (
                  <div className="flex flex-col items-start  w-full max-md:pr-5 max-md:max-w-full border-b-2 gap-2 p-2">
                    <div className="flex justify-between w-full">
                      <div className="flex gap-2  text-xs text-zinc-500">
                        <span className="flex justify-center items-center object-contain shrink-0 w-7 h-7 rounded-full aspect-square bg-primary text-white text-sm p-1 ">
                          {review.name.slice(0, 1).toUpperCase()}
                        </span>
                        <div className="my-auto basis-auto">{review.name}</div>
                      </div>
                      <button
                        onClick={async () => {
                          const res = await axios.delete(
                            `${PROXY}/review/${review._id}`,
                            {
                              headers: {
                                Authorization: `Bearer ${
                        ...
                        className="flex justify-center items-center object-contain shrink-0 w-7 h-7 rounded-full aspect-square bg-primary text-white text-sm p-1 "
                      >
                        <DeleteFilled />
                      </button>
                    </div>
                    <div className="flex justify-start items-center gap-2 w-full overflow-auto ">
                      {review.pics.map((img: string, index: number) => {
                        return (
                          <img
                            src={`${S3PROXY}${img}`}
                            loading="lazy"
                    ...
                    <div className="flex gap-2 justify-center items-center">
                      <div className="shrink-0 flex h-6   gap-1 p-1 text-sm font-semibold leading-none text-white whitespace-nowrap bg-primary rounded items-center justify-center">
                        <span>{review.rating}</span>
                        <img
                          loading="lazy"
                          src="https://cdn.builder.io/api/v1/image/assets/TEMP/7ba8c24a6b378203d4b348c8200aca3dc854b6108014273e32a82b31acbebee8?placeholderIfAbsent=true&apiKey=76360ea64ff84b92bea2f9d08dedb9ab"
                          alt="Rating star icon"
                          className="object-contain shrink-0  aspect-[0.95] w-[12px]"
                        />
                      </div>
                      <div>
                        ...
                        <p className="mt-3.5 text-sm leading-none text-slate-900 max-md:max-w-full">
                          {review.reviewBody}
                        </p>
                      </div>
                    </div>
